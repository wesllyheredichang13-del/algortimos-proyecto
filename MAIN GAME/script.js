// Quitar Diacriticos
    ...
    // Desde aqui se agarran los elementeos para manipular
    ...
    // Esto inicia el teclado en pantalla
    const makeKey = (ch) => {
    ...
    // MAIN (LO mero principal)

    const urlParams = new URLSearchParams(window.location.search);
    const nivel = urlParams.get('nivel');  // <- preguntar que dificultad usaremos
  ...
  $lives.textContent = String(MAX_ERRORS - errors); // vidas
  $mistakes.textContent = String(errors); // errores
  $hint.textContent = `${secretOriginal.length} letras`; // pista

  // palabra
  $word.innerHTML = '';

  function mostrarPalabra(idx) {
    if (idx >= secretOriginal.length) return;
    const char = secretOriginal[idx];
  ...
  mostrarPalabra(0);
// letras reveladas
function contarLetrasReveladas(idx = 0, count = 0) {
  if (idx >= secretOriginal.length) return count;
  if (isRevealed(idx)) count++;
  return contarLetrasReveladas(idx + 1, count);
}
  ...
  if (isWin()) {
    ...
    disableAllKeys(true);
    // Mostrar el botón de "Volver a Jugar" también al perder
    document.getElementById('restartGame').style.display = 'block';
  }
